...
% Il se peut que la 38 eme figure ne s'affiche pas au premier essai
% veuillez alors relancer le script. Bonne lecture !
...
disp(['Erreur maximale pour la base d entrainement (k=30) : ' num2str(max(E_train))]);
disp(['Erreur moyenne pour la base d entrainement (k=30) : ' num2str(mean(E_train))]);
...
disp(['Erreur maximale pour la base de test (k=30) : ' num2str(max(E_test))]);
disp(['Erreur moyenne pour la base de test (k=30) : ' num2str(mean(E_test))]);
